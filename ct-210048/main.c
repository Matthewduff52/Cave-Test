...
int main(void) {
    ...
    while (!key_map[1]) {
            ...
            for (x = 0; x < SCREENW; x += step) {
                ...
                if (hit) {
                    ...
                    if (worldMap[mX][mY][mZ] == 1) { //Grass block
                        if (side == 0) color = 0x72; // dark side
                        if (side == 1) color = 0x06; // light side
                        if (side == 2) color = 0x2F; // top
                    }

                    if (worldMap[mX][mY][mZ] == 2) { // Stone block
                        if (side == 0) color = 0x1A;
                        if (side == 1) color = 0x1C;
                        if (side == 2) color = 0x1E;
                    }

                    if (worldMap[mX][mY][mZ] == 3) { // Wood block
                        if (side == 0) color = 0x43;
                        if (side == 1) color = 0x44;
                        if (side == 2) color = 0x5C;
                    }
        ...
        if (key_map[0x02]) {inventorySelect = 1; sprintf(invMsg, "Grass", inventorySelect);} // Row 1, Column 1
        if (key_map[0x03]) {inventorySelect = 2; sprintf(invMsg, "Stone", inventorySelect);} // Row 1, Column 1
        if (key_map[0x04]) {inventorySelect = 3; sprintf(invMsg, "Wood", inventorySelect);} // Row 1, Column 1
