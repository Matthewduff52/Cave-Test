...
double posX = 12.0, posY = 12.0, posZ = 60;
...
int main(void) {
    ...
    int x, y, z, mX, mY, mZ, stX, stY, stZ, hit, side, h;
    float oldDirX, oldPlaneX, angle;
    int mdx, mdy;
    float mouseSensitivity = 0.005f;
    float pitchSensitivity = 2.0f;
    double moveSpeed = 0.05;
    float camX, camY, rDirX, rDirY, rDirZ;
    float dDX, dDY, dDZ, sDX, sDY, sDZ;
    unsigned char color;
    ...
    int breakTimer = 0;
    int buildTimer = 0;
    int maxRenderDist = 32; // Distance in blocks
    int steps = 0;
    ...
    for(x=0; x<wX; x++) {
        for(y=0; y<wY; y++) {
            // Generate a height using sine waves
            // Adding two different frequencies makes it look less "mechanical"
            h = baseHeight + (int)(sin(x * frequency) * amplitude) + (int)(cos(y * frequency * 0.5) * amplitude * 0.5);

            // Clamp height to map limits
            if (h >= wZ) h = wZ - 1;
            if (h < 1) h = 1;

            // 1. Set the shadowMap immediately
            shadowMap[x][y] = h;

            // 2. Fill the columns in the worldMap
            for (z = 0; z < wZ; z++) {
                if (z < h) {
                    worldMap[x][y][z] = 2; // Stone underground
                } else if (z == h && z >= baseHeight) {
                    worldMap[x][y][z] = 1; // Grass on top
                } else if (z == h && z < baseHeight) {
                    worldMap[x][y][z] = 4; // Dirt on top
                } else {
                    worldMap[x][y][z] = 0; // Air
                }
            }
            /*worldMap[x][y][0] = 2;
            worldMap[x][y][1] = 2;
